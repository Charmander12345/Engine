...
- `tools/bootstrap.ps1 (Ninja)`: Gleicher Fix für Ninja — System-Ninja auf PATH verhindert nicht mehr den lokalen Download nach `Tools/ninja/`. Stellt sicher, dass der Editor Ninja immer unter dem erwarteten Pfad findet.
- **Hintergrund:** `detectCMake()` (BuildSystemUI.cpp) sucht zuerst nach dem gebündelten CMake in `Tools/cmake/bin/cmake[.exe]`, dann VS via vswhere, dann System PATH, dann Common Install Paths. Wenn Bootstrap nur auf System-CMake verließ und der Editor-Prozess kein PATH-cmake hatte, wurde kein CMake gefunden und der C++-Script-Build beim PIE-Start übersprungen.

## Aktuelle Änderung (Key-Callback Entity-Routing Fix)

- `Key-Callback Entity-Fix (InputModule.cpp)`: Per-Key-Callbacks (`register_key_pressed`/`register_key_released`) übergaben bisher den **Key-Code** als Parameter an die Callback-Funktion. Der Benutzer erwartet aber die **Entity**, die den Callback registriert hat (konsistent mit `onloaded(entity)`, `tick(entity, dt)`, `on_entity_begin_overlap(entity, other)`). **Fix:** Neues `EntityCallback`-Struct speichert `PyObject* callback` + `unsigned long entity`. `s_currentEntity` Context-Variable wird in `PythonScripting.cpp` vor `onloaded`/`tick`/overlap/cross-call gesetzt. `InvokeKeyCallbacksForKey` übergibt jetzt `entity` statt `key` an den Callback. Globale Callbacks (`set_on_key_pressed`/`set_on_key_released`) bleiben unverändert (übergeben weiterhin Key-Code).
- `ScriptingInternal.h`: `EntityCallback`-Struct + `s_currentEntity` extern. Per-Key-Maps von `vector<PyObject*>` auf `vector<EntityCallback>` umgestellt. `InvokeKeyCallbacksForKey` Signatur aktualisiert.
- `engine.pyi`: Docstrings für `register_key_pressed`/`register_key_released` aktualisiert — Callback erhält Entity, nicht Key-Code.

## Aktuelle Änderung (Unified Script Call System – callFunction auto-routes C++ ↔ Python)

- `Unified callFunction API`: Neues `callFunction(entity, funcName, args)` — ein einziger Aufruf routet automatisch zum C++-Script (`onScriptCall`) oder Python-Script der Ziel-Entity. Aufrufer müssen nicht wissen, in welcher Sprache die Logik implementiert ist.
- **NativeScriptManager (callFunction):** Neue Methode `callFunction(Entity, funcName, args)` — prüft zuerst `m_instances` (C++ INativeScript), fällt bei None-Rückgabe auf `m_pythonCallHandler` (Python) zurück. Beide Sprachen werden über eine einzige Schnittstelle abgedeckt.
- **GameplayAPI (callFunction):** Neue freie Funktion `GameplayAPI::callFunction()` delegiert an `NativeScriptManager::callFunction()`.
- **INativeScript (callFunction):** Neue statische Convenience-Methode `INativeScript::callFunction(entity, funcName, args)` — C++-Gameplay-Scripts können damit Funktionen auf beliebigen Entities aufrufen, unabhängig von der Sprache. Bestehende `callScriptFunction` (nur C++) und `callPythonFunctionOn` (nur Python) bleiben als Legacy erhalten.
- **Python call_function Binding:** Neue Python-Funktion `engine.entity.call_function(entity, func_name, *args)` — routet automatisch zu C++ oder Python. `call_native` bleibt für expliziten C++-Zugriff erhalten.
- **engine.pyi:** `call_function(entity, func_name, *args) -> object` Stub hinzugefügt.
- **Architektur:** Routing-Logik zentral in `NativeScriptManager::callFunction()`: C++ `onScriptCall()` wird bevorzugt, bei `ScriptValue::None` wird Python probiert. Kein Wissen über Zielsprache erforderlich.

## Aktuelle Änderung (Scripting-Kommunikation Bugfixes – call_native, callPythonFunction, Template-Namen)

- `call_native Python→C++ Variadic Args Fix`: `engine.entity.call_native(entity, func_name, arg1, ...)` schlug mit "too many arguments" fehl, da `PyArg_ParseTuple(args, "ks", ...)` keine Extra-Args erlaubt. **Fix (EntityModule.cpp):** `PyArg_ParseTuple` durch manuelles `PyTuple_GetItem` für entity+funcName ersetzt. Variadic Args werden weiterhin ab Index 2 gesammelt.
- `callPythonFunction C++→Python Silent-Failure Logging`: `callPythonFunction()` aus C++-Scripts gab keine Fehlermeldung wenn die Entity kein Python-Script hatte, das Script nicht geladen war, oder die Funktion nicht existierte. **Fix (PythonScripting.cpp):** `HandleCppCallsPython()` loggt jetzt `Logger::WARNING` bei jedem Early-Exit (kein Interpreter, kein LogicComponent, Script nicht geladen, Funktion nicht gefunden).
- `Python-Script-Template Funktionsnamen-Mismatch`: AssetManagerEditorWidgets.cpp und OutlinerPanel.cpp generierten `def on_loaded(entity)` und `def on_begin_overlap(entity, other)` — PythonScripting.cpp sucht aber nach `onloaded` und `on_entity_begin_overlap`/`on_entity_end_overlap`. **Fix:** Templates korrigiert zu `onloaded`, `on_entity_begin_overlap`, `on_entity_end_overlap` (identisch mit EditorApp.cpp-Template).

## Aktuelle Änderung (Erweiterte C++ & Python Scripting APIs – Scene Queries, Cross-Entity, Cross-Script)

- `Erweiterte Scripting APIs`: C++ (GameplayAPI/INativeScript) und Python (engine.entity) um umfangreiche Scene-Query-, Cross-Entity- und Cross-Script-Funktionen erweitert.
- **Entity Queries (C++ & Python):** `isEntityValid(entity)`, `hasComponent(entity, kind)`, `getEntityName(entity)`/`setEntityName(entity, name)`, `getEntityCount()`, `getAllEntities()`, `findEntityByName(name)` (war nur C++, jetzt auch Python), `distanceBetween(entityA, entityB)` (Euklidische Distanz via TransformComponent).
- **Cross-Entity Transform Access (C++):** `getPositionOf/setPositionOf`, `getRotationOf/setRotationOf`, `getScaleOf/setScaleOf` — Zugriff auf Transform anderer Entities direkt aus INativeScript (delegiert an bestehende GameplayAPI-Funktionen).
- **Cross-Entity Physics Access (C++ & Python):** `getVelocityOf/setVelocityOf` (C++), `get_velocity/set_velocity/add_force/add_impulse` (Python entity-Modul) — Physics-Zugriff auf beliebige Entities.
- **Cross-Script Communication (C++ & Python):** `callScriptFunction(entity, funcName, args)` — ruft `onScriptCall()` auf dem C++-Script einer anderen Entity auf. `callPythonFunctionOn(entity, funcName, args)` — ruft Python-Funktion auf beliebiger Entity auf (nicht nur eigene). Beide Richtungen (C++→C++, C++→Python, Python→C++) funktional.
- **Python entity-Modul erweitert:** 14 neue Funktionen: `remove_entity`, `has_component`, `find_entity_by_name`, `get_entity_name`, `set_entity_name`, `is_entity_valid`, `get_entity_count`, `get_all_entities`, `distance_between`, `get_velocity`, `set_velocity`, `add_force`, `add_impulse`.
- **engine.pyi aktualisiert:** Alle neuen Python-Funktionen mit Typ-Annotationen und Docstrings.
- **Dateien:** `GameplayAPI.h` (neue Deklarationen), `GameplayAPI.cpp` (Implementierungen + INativeScript-Delegates), `INativeScript.h` (neue Convenience-Methoden), `EntityModule.cpp` (Python-Bindings), `engine.pyi` (IntelliSense-Stubs).
...
- `main.cpp Integration (Phase 3)`: (1) Runtime-Modus: `GameScripts.dll` aus `Engine/`-Verzeichnis laden via `loadGameplayDLL()`. `initializeScripts()` wird **deferred** im Game-Loop aufgerufen — erst nachdem `isScenePrepared()` true ist und ECS-Entities aus dem Level-JSON erstellt wurden. (2) Kamera-Zuweisung ebenfalls deferred nach Scene-Preparation. (3) Game-Loop: `NativeScriptManager::updateScripts(dt)` vor Physics und Python-Scripts (C++ → Physics → Python Lifecycle-Reihenfolge). (4) Shutdown: `shutdownScripts()` + `unloadGameplayDLL()` vor `Scripting::Shutdown()`.
...
- `Auto-Build GameScripts.dll vor PIE`: Automatische Kompilierung von C++-Game-Scripts vor jedem Play-In-Editor-Start. **Async Build mit Popup:** `buildGameScriptsForPIE()` läuft jetzt asynchron — Pre-Build-Setup (Auto-Register, CMakeLists.txt-Generierung, DLL-Unload) auf dem Main-Thread, dann wird der eigentliche CMake-Build in einem separaten `std::thread` ausgeführt. Ein separates Popup-Fenster ("C++ Script Build") zeigt den Build-Output live an mit scrollbarem Log-Panel, Fehler-Highlighting (rot) und Auto-Scroll. **Bei Erfolg:** Popup schließt sich automatisch, DLL wird geladen, PIE startet via `finishStartPIE()`. **Bei Fehler:** Popup bleibt offen mit roter Status-Meldung und Close-Button — Benutzer kann den kompletten Build-Output mit Fehlermeldungen durchscrollen. `pollPIEBuild()` wird in `EditorApp::tick()` aufgerufen und draint pending Zeilen vom Build-Thread in die UI. `dismissPIEBuildPopup()` schließt Popup und räumt State auf. **EditorApp.h:** Neue Member: `m_pieBuildThread`, `m_pieBuildMutex`, `m_pieBuildRunning` (atomic), `m_pieBuildPendingLines`, `m_pieBuildFinished`, `m_pieBuildSuccess`, `m_pieBuildOutputLines`, `m_pieBuildWidget`, `m_pieBuildPopup`. Neue Methoden: `finishStartPIE()`, `pollPIEBuild()`, `dismissPIEBuildPopup()`. CMakeLists.txt wird bei jedem Aufruf überschrieben um Engine-Pfade aktuell zu halten. Build-Cache in `<projectPath>/Binary/GameScripts/` für inkrementelle Builds. **PIE-Lifecycle-Fix:** `stopPIE()` ruft `NativeScriptManager::shutdownScripts()` + `unloadGameplayDLL()` auf bevor ECS-Snapshot wiederhergestellt wird.
- `Native Script Class Discovery & Entity Assignment`: **NativeScriptManager** cached entdeckte Klassennamen (`m_cachedClassNames`) nach DLL-Laden – bleiben auch nach Unload verfügbar. `getAvailableClassNames()` liefert gecachte Liste. **Verbessertes Logging:** Bei 0 registrierten Klassen wird detaillierte Warnung mit `REGISTER_NATIVE_SCRIPT`-Hinweis geloggt; `initializeScripts()` loggt jede Entity-Klassen-Zuordnung und warnt wenn keine Entities ein nativeClassName gesetzt haben. **EntityDetails C++ Class Dropdown:** OutlinerPanel LogicComponent-Abschnitt hat neues DropdownButtonWidget (`Details.Logic.CppClassDropdown`) — zeigt alle verfügbaren C++-Klassen aus NativeScriptRegistry, mit "(None)"-Option zum Zurücksetzen und "(Build C++ scripts first)"-Hinweis wenn keine Klassen verfügbar. Undo/Redo-Support. **UIManager Bridge:** `NativeClassNamesProvider`-Callback-Pattern (`setNativeClassNamesProvider`/`getAvailableNativeClassNames`) überbrückt DLL-Grenze Renderer↔Scripting (gleiche Architektur wie Drop-Callbacks). EditorApp setzt den Provider im `initialize()`.
- `Auto-Registration & New C++ Script Template`: **Auto-Registration:** `buildGameScriptsForPIE()` scannt vor dem CMake-Build alle `.h`-Dateien in `Content/Scripts/Native/` per Regex (`class X : public INativeScript`) und generiert automatisch `_AutoRegister.cpp` mit `#include`-Direktiven und `REGISTER_NATIVE_SCRIPT`-Makros für jede entdeckte Klasse. Benutzer müssen `REGISTER_NATIVE_SCRIPT` nie manuell schreiben — die Engine erledigt das transparent. Die generierte Datei wird bei jedem Build überschrieben und von `file(GLOB *.cpp)` im CMakeLists.txt erfasst. **New C++ Script Kontextmenü:** Content Browser Rechtsklick-Menü hat neuen Eintrag "New C++ Script". Erstellt `.h`- und `.cpp`-Boilerplate mit `INativeScript`-Vererbung und `onLoaded()`/`tick()`-Overrides in `Content/Scripts/Native/` (unabhängig vom aktuellen Browser-Ordner). Automatische Namenskollisionsvermeidung (`NewScript`, `NewScript1`, `NewScript2` etc.).
- `GameScripts im Game Build`: **BuildPipeline.cpp** hat neuen Step 7 "Build C++ GameScripts DLL" (kTotalSteps von 9 auf 10). Prüft ob `Content/Scripts/Native/` im Projekt `.cpp`-Dateien enthält. Falls ja: (1) Auto-Registration-Scan (identische Regex wie PIE-Build) generiert `_AutoRegister.cpp`, (2) CMakeLists.txt wird generiert (C++20, SHARED, linkt gegen `ScriptingRuntime.lib` aus `Tools/lib/`, NativeScripting-Headers aus `Tools/include/`), (3) CMake configure + build im `<binaryDir>/GameScripts/`-Unterordner, (4) `GameScripts.dll` wird nach `<outputDir>/Engine/` deployed, PDB nach `Symbols/` (non-Shipping). Falls keine C++-Scripts vorhanden, wird der Step übersprungen. **Runtime-Laden (main.cpp):** Nach Level-Load sucht die Runtime `GameScripts.dll` in `Engine/` und lädt sie via `NativeScriptManager::loadGameplayDLL()` + `initializeScripts()`. `SetDllDirectoryW(Engine/)` stellt sicher, dass abhängige DLLs (`ScriptingRuntime.dll` etc.) gefunden werden. **DLL-Link-Fix:** GameScripts linkt gegen `ScriptingRuntime` (nicht `Scripting`), da im Game-Build nur `ScriptingRuntime.dll` deployed wird — `Scripting.dll` ist editor-only. Log-Meldung wenn DLL nicht gefunden wird.
- `Script Method Auto-Registration (SCRIPT_METHOD Makro)`: Template-basiertes System in `INativeScript.h` — C++ Methoden mit beliebigen Signaturen können per `SCRIPT_METHOD(methodName)` in `onLoaded()` registriert und dann von Python per `call_function(entity, "methodName", args...)` aufgerufen werden. `registerMethod(name, &Class::method)` deduziert Parametertypen (float, int, bool, string, ScriptValue) via C++20 Member-Function-Pointer-Templates und `if constexpr`. `std::index_sequence` entpackt `vector<ScriptValue>` automatisch in typisierte Argumente. Void-Methoden geben `Bool(true)` zurück um Fall-Through zu Python zu verhindern. `onScriptCall`-Default dispatcht aus `m_scriptMethods`-Map — manuelles Override weiterhin möglich.
- `VS Code IntelliSense für C++ GameScripts`: `EditorApp::generateVSCodeConfig()` generiert `.vscode/c_cpp_properties.json` im `Content/Scripts/`-Unterverzeichnis des Projekts. Include-Pfade: `${workspaceFolder}/Native/**` + `<EditorBinDir>/Tools/include/NativeScripting`. Defines: `GAMEPLAY_EXPORTS`. Standard: C++20. IntelliSense-Modus: `windows-msvc-x64`. Automatisch aufgerufen bei "New C++ Script" (Content-Browser-Kontextmenü) und `buildGameScriptsForPIE()`. Benutzer öffnet `Content/Scripts/` in VS Code → volle IntelliSense für INativeScript, GameplayAPI, ScriptValue, SCRIPT_METHOD etc.
- `INativeScript::callFunction (Non-Static Overload)`: Neue nicht-statische Convenience-Methode `callFunction(funcName, args)` — delegiert an die statische `callFunction(m_entity, funcName, args)`. Ermöglicht in C++-Scripts `callFunction("doSomething")` statt `callFunction(getEntity(), "doSomething")` für Aufrufe auf der eigenen Entity. Statische Version `callFunction(entity, funcName, args)` bleibt für Cross-Entity-Aufrufe.

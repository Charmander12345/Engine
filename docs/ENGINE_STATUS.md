...
- ✅ `OpenGLRendererTabs.cpp – Tab-System Split`: 7 Editor-Tab-Methoden in `OpenGLRendererTabs.cpp` (294 Zeilen) verschoben: `addTab`, `removeTab`, `rebuildTitleBarTabs`, `setActiveTab`, `getActiveTabId`, `getTabs`, `releaseAllTabFbos`. `getEntityRotationMatrix` bleibt im Hauptfile (shared). **CMakeLists.txt:** Beide neue Dateien zu `OPENGL_RENDERER_SOURCES` hinzugefügt. **Ergebnis:** `OpenGLRenderer.cpp` von ~11.018 auf ~9.906 Zeilen reduziert (-1.112 Zeilen).

## Letzte Änderung (Codebase-Cleanup 2.2: Debug-Rendering extrahiert)

- ✅ `OpenGLRendererDebug.cpp – Debug-Rendering Split`:

## Letzte Änderung (Codebase-Cleanup 2.1: UI-Rendering vereinheitlicht)

- ✅ `renderWidgetElement – Unified UI Rendering`: Drei nahezu identische `renderElement`-Lambdas (~450–800 Zeilen je) in `drawUIWidgetsToFramebuffer`, `renderViewportUI` und `renderUI` (OpenGLRenderer.cpp) durch eine einzige `renderWidgetElement()`-Methode (~665 Zeilen) ersetzt. **Neue Strukturen (OpenGLRenderer.h):** `UIRenderContext`-Struct mit `projection` (glm::mat4), `screenHeight` (int), `debugEnabled` (bool), `scissorOffset` (Vec2, Default {0,0}), `DeferredDropDown`-Substruct + Vektor-Pointer. Die drei Rendering-Methoden sind jetzt dünne Wrapper (5–7 Zeilen UIRenderContext-Setup + Widget-Iteration). `scissorOffset` löst den Viewport-UI-Offset (Scissor relativ zu Fenster, Widgets relativ zu Viewport-Rect). RAII-Structs `RtRestore`/`ScissorRestore` innerhalb der Methode für rekursive Aufrufe. Widget-Editor-Preview-FBO nutzt separaten `previewCtx`. **Ergebnis:** `OpenGLRenderer.cpp` von ~12.787 auf ~11.563 Zeilen reduziert (-1.224 Zeilen).

## Letzte Änderung (PythonScripting Module Split – Phase 2: Alle Module extrahiert)

- ✅ `PythonScripting Module Split (Phase 2)`: Alle verbleibenden 9 Python-Submodule aus der monolithischen `PythonScripting.cpp` in eigenständige Übersetzungseinheiten extrahiert. **PythonScripting.cpp** von ~3.690 auf 1.380 Zeilen reduziert (~63% Reduktion). **Neue Dateien:** `EntityModule.cpp` (468 Zeilen, 14 Funktionen: create_entity, attach/detach_component, get_entities, get/set_transform/position/rotation/scale, translate, rotate, set/get_mesh, get/set_light_color), `AudioModule.cpp` (354 Zeilen, 15 Funktionen: create/play/pause/stop/invalidate_audio, volume, handle-basiert + pfad-basiert + asset-basiert + async), `InputModule.cpp` (307 Zeilen, 8 Funktionen + Shared State: set_on_key_pressed/released, register_key_pressed/released, is_shift/ctrl/alt_pressed, get_key; definiert `ScriptDetail::s_onKeyPressed/Released`, `s_keyPressedCallbacks/ReleasedCallbacks`, `ClearKeyCallbacks()`, `InvokeKeyCallbacks/ForKey()`, `PopulateKeyConstants()`), `CameraModule.cpp` (213 Zeilen, 13 Funktionen: get/set_camera_position/rotation, transition_to/is_transitioning/cancel_transition, start/pause/resume/stop_path, is_path_playing, get_path_progress), `UIModule.cpp` (489 Zeilen, 16 Funktionen: show_modal/close_modal/show_toast, spawn/remove_widget, play/stop/set_speed_animation, show_cursor, clear_all_widgets, set/clear/get_focus, set_focusable, set_draggable, set_drop_target), `ParticleModule.cpp` (98 Zeilen, 4 Funktionen: set_emitter, set_enabled, set_color, set_end_color), `DiagnosticsModule.cpp` (126 Zeilen, 8 Funktionen: get_delta_time, get_engine_time, get/set_state, get_cpu/gpu/ram/monitor_info), `LoggingModule.cpp` (53 Zeilen, 1 Funktion: log mit level-Parameter), `EditorModule.cpp` (393 Zeilen, 8 Funktionen + Plugin-Management: show_toast, get_selected_entities, get_asset_list, create/select_entity, add_menu_item, register_tab, get_menu_items; enthält jetzt auch LoadEditorPlugins/PollPluginHotReload/GetPluginMenuItems/GetPluginTabs/InvokePluginMenuCallback). **ScriptingInternal.h** erweitert um 9 neue `Create*Module()`-Forward-Deklarationen + Input-Callback-State/Funktions-Deklarationen. **PythonScripting.cpp:** `CreateEngineModule()` nutzt Factory-Calls für alle 11 Module (nur `AssetModule` bleibt lokal). `HandleKeyDown/HandleKeyUp` nutzen `ScriptDetail::`-Prefix. `Shutdown/ReloadScripts` rufen `ScriptDetail::ClearKeyCallbacks()`. Editor-Plugin-Management komplett nach `EditorModule.cpp` verschoben. **CMakeLists.txt:** 9 neue `.cpp`-Dateien zu `SCRIPTING_SOURCES` hinzugefügt. Beide Targets (Scripting + ScriptingRuntime) kompilieren fehlerfrei.
...
- ✅ `Content Browser Suche & Filter (Phase 4.2)`: Echtzeit-Suchfeld und Typ-Filter im Content Browser. `m_browserSearchText` (string) + `m_browserTypeFilter` (uint32_t Bitmask) in `ContentBrowserPanel`. PathBar erweitert: Typ-Filter-Toggle-Buttons (Mesh/Mat/Tex/Script/C++ Script/Audio/Level/Widget/Prefab/Input Action/Input Mapping/Actor) als Accent-farbige Toggles + EntryBar-Suchfeld rechts. Suchmodus: Bei nicht-leerem Suchtext werden ALLE Assets über alle Ordner als flache Liste angezeigt (case-insensitive Substring-Suche + Typ-Filter). Normalmodus: Typ-Filter filtert Assets im aktuellen Ordner. Doppelklick auf Such-Ergebnis: navigiert zum Asset-Ordner, klappt Tree auf, öffnet Asset-Editor (Model3D→MeshViewer, Widget→WidgetEditor, Material→MaterialEditor, Level→LevelLoad, ActorAsset→ActorEditorTab), leert Suchtext. `focusContentBrowserSearch()` Methode + Ctrl+F Shortcut in main.cpp fokussiert das Suchfeld. Actor Assets werden mit eigenem Icon-Tint dargestellt und können inline umbenannt werden (Rename-Button); beim Umbenennen eines Actors werden die eingebetteten Script-Dateien (.h/.cpp) und der Script-Klassenname automatisch mit aktualisiert.

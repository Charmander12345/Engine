...
#   endif
    ...
    if (renderer && !isRuntimeMode)
    {
        ...
        // --- Build Game callback (Phase 10) -- extracted to BuildPipeline ---
        renderer->getUIManager().setOnBuildGame([&renderer](const UIManager::BuildGameConfig& config)
            {
                BuildPipeline::execute(config, renderer->getUIManager(), renderer);
            });

...
namespace
{
    bool WriteTextFileIfDifferent(const std::filesystem::path& path, const std::string& content)
    {
    ...
    std::string BuildNativeScriptsFingerprint(const std::filesystem::path& nativeDir)
    {
        namespace fs = std::filesystem;

        std::vector<std::string> entries;
        std::error_code ec;
        for (const auto& file : fs::directory_iterator(nativeDir, ec))
        {
            ...
            if (file.path().filename() == "_AutoRegister.cpp")
                continue;

            std::error_code timeEc;
            const auto relPath = file.path().filename().generic_string();
            const auto fileSize = fs::file_size(file.path(), timeEc);
            if (timeEc)
                continue;
            const auto writeTime = fs::last_write_time(file.path(), timeEc);
            if (timeEc)
                continue;

            entries.push_back(relPath + "|" + std::to_string(fileSize) + "|" +
...
void EditorApp::buildGameScriptsForPIE()
{
    ...
    const fs::path fingerprintPath = buildDir / ".pie_native_build_fingerprint";
    ...
    const std::string currentFingerprint = BuildNativeScriptsFingerprint(nativeDir);
    if (fs::exists(dllPath) && fs::exists(fingerprintPath))
    {
        std::ifstream fingerprintIn(fingerprintPath, std::ios::binary);
        if (fingerprintIn.is_open())
        {
            std::ostringstream previousFingerprint;
            previousFingerprint << fingerprintIn.rdbuf();
            if (previousFingerprint.str() == currentFingerprint)
            {
                logger.log(Logger::Category::Engine,
                    "PIE build: native C++ scripts unchanged – reusing previous GameScripts build.",
                    Logger::LogLevel::INFO);
                uiMgr.showToastMessage("C++ scripts unchanged - skipping rebuild.", UIManager::kToastShort,
                    UIManager::NotificationLevel::Info);
                finishStartPIE();
                return;
            }
        }
    ...
    // ── Auto-generate _AutoRegister.cpp ────────────────────────────────
    {
        ...
        if (WriteTextFileIfDifferent(autoRegPath, autoReg.str()))
        {
            if (!discovered.empty())
            {
                logger.log(Logger::Category::Engine,
    ...
    // ── Generate CMakeLists.txt ──────────────────────────────────────
    {
        ...
        WriteTextFileIfDifferent(cmakeListsPath, cmake.str());
    }
    ...
    m_pieBuildFingerprint = currentFingerprint;
...
void EditorApp::generateVSCodeConfig(const std::string& projectPath)
{
    ...
    if (WriteTextFileIfDifferent(configPath, json.str()))
    {
...
void EditorApp::pollPIEBuild()
{
    ...
    if (finished)
    {
        ...
        if (success)
        {
            ...
            {
                namespace fs = std::filesystem;
                const fs::path fingerprintPath = fs::path(projectPath) / "Binary" / "GameScripts" / ".pie_native_build_fingerprint";
                WriteTextFileIfDifferent(fingerprintPath, m_pieBuildFingerprint);

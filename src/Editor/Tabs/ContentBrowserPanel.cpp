    ...
    if (gridPanel)
    {
        ...
        else if (isSearchMode)
        {
            ...
            for (const auto& item : gridAssets)
            {
                ...
                tile.onDoubleClicked = [this, relPath, assetType = item.type]()
                {
                    ...
                    {
                        if (m_tabOpener) m_tabOpener->openWidgetEditorPopup(relPath);
                    }
                    ...
                    {
                        if (m_tabOpener) m_tabOpener->openAudioPreview(relPath);
                    }
        ...
        for (const auto& item : gridAssets)
        {
            ...
            tile.onDoubleClicked = [this, relPath, assetType = item.type]()
            {
                ...
                {
                    if (m_tabOpener) m_tabOpener->openEntityEditor(relPath);
                    return;
                }
                if (assetType == AssetType::ActorAsset)
                {
                    if (m_tabOpener) m_tabOpener->openActorEditor(relPath);
                    return;
                }
                if (assetType == AssetType::InputAction)
                {
                    if (m_tabOpener) m_tabOpener->openInputActionEditor(relPath);
                    return;
                }
                if (assetType == AssetType::InputMapping)
                {
                    if (m_tabOpener) m_tabOpener->openInputMappingEditor(relPath);
                    return;
                }

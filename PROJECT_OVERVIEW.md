...
The editor provides a dedicated **Skeletal Mesh Editor** tab (`src/Editor/Tabs/SkeletalMeshEditorTab`) for viewing and inspecting skeletal mesh assets:

- **Hierarchical Bone Tree View** (left panel) – Displays the bone hierarchy as an expandable/collapsible tree structure. Bones are organized by parent-child relationships derived from the mesh's node hierarchy. Each bone entry shows its name and parent reference for spatial context.
- **Statistics Panel** (right panel) – Shows mesh metadata: asset path, vertex count, index count, sub-mesh count, skeletal flag, bone count, and skinned vertex count. Provides read-only inspection of the mesh's construction.
- **Animation List** (right panel) – Displays all animation clips embedded in the skeletal mesh asset, showing name, duration in seconds, and channel count for each clip. Useful for verifying import completeness.
- **Asset-Centric Design** – Skeletal meshes are viewed as self-contained assets, not applied to other entities. When placing a skeletal mesh in the world, a new actor is spawned from the skeletal mesh definition rather than attaching it to an existing entity.
